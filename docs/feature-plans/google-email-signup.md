...
1. **Single-user → multi-user pivot.** CLAUDE.md states this is a "single-user MVP" — the entire codebase assumes exactly one hardcoded `User` row (seeded via `prisma/seed.ts` / created via `prisma:bootstrap-admin`), and every service already scopes every query by `userId` (so the data-access layer is *already* multi-tenant-safe). A real "sign up" flow implies **anyone can create an account**, which is a product pivot, not just an auth mechanism swap. Two ways to satisfy the ask:
   ...
   - **(B) Stay single-user, add alternate login methods only.** No public sign-up; the one seeded user can *also* log in via Google (linked to their existing email) instead of only credentials. "Sign up with name" wouldn't apply here.

...
- [ ] Raw key is never logged — no `console.log`/`console.error` of the input,
      the decrypted value, or any object containing it. Verify by review of both
      services and both adapters.
- [ ] Raw key never appears in a thrown `Error` message, a `cause`, or a stack —
      adapters classify by status code and throw the typed errors from
      `lib/ai/errors.ts`, never `throw new Error(responseBody)`.
- [ ] Provider response bodies are never forwarded to the client verbatim.
- [ ] `GET /api/settings/ai` returns only `maskedKey` from `keyLast4`. There is
...
- [ ] `PATCH /toggles` takes no key field, so a toggle flip can never overwrite
      or re-submit the stored key.
- [ ] Key excluded from data export (by omission) — asserted by test.
- [ ] Key survives data import — asserted by test (the `wipeUserData` trap).
- [ ] Key wiped on account deletion via `onDelete: Cascade` — asserted by test.
- [ ] `SECRET_ENCRYPTION_KEY` is server-only, never `NEXT_PUBLIC_`, never sent
      to the client, absent from the export and from any API response.
- [ ] GCM AAD is the `userId`, so a blob cannot be replayed into another row.
- [ ] Every AI route is session-gated and every query is `where: { userId }`;
      `transactionId` is never trusted without the `userId` scope.
- [ ] DB-backed per-user daily cap on `POST /api/categorize/suggest-ai`,
...
- [ ] Only `payee`, `type`, category names/ids, and opt-in `note`/`amount` leave
      ...
      prompt-builder unit test that snapshots the exact outbound body.
- [ ] Disclosure preview is generated by the same builder as the real request,
      so it cannot drift from reality.
- [ ] Suggest endpoint never writes to `Transaction`; applying a suggestion goes
...
- [ ] **Get explicit user authorization for the `prisma/schema.prisma` change** before anything else; confirm the exact `UserAiSettings` model and `AiProvider` enum above
- [ ] Add `AiProvider` enum, `UserAiSettings` model and `User.aiSettings` back-relation to `prisma/schema.prisma`
- [ ] `npm run prisma:migrate -- --name add_user_ai_settings` and `npm run prisma:generate`
- [ ] Add `SECRET_ENCRYPTION_KEY` to `.env.example` and README setup docs
- [ ] Implement `lib/crypto/secrets.ts` (encrypt/decrypt/mask/`isSecretEncryptionConfigured`, `v1:` packing, userId AAD)
- [ ] Unit tests: `tests/unit/lib/secrets.test.ts` — round-trip, wrong-AAD rejection, tampered-tag rejection, unconfigured throw
- [ ] Implement `lib/ai/types.ts` and `lib/ai/errors.ts` (taxonomy + `aiErrorToResponse`)
- [ ] Implement `lib/ai/prompt.ts` (system prompt, `buildSuggestionPayload`, category-id enum construction)
- [ ] Unit tests: `tests/unit/lib/ai-prompt.test.ts` — note/amount omitted unless opted in; no date/account/flags in payload; enum includes `"none"`
- [ ] Implement `lib/ai/anthropic.ts` (forced tool use) and `lib/ai/openai.ts` (strict json_schema), both with status→typed-error classification
- [ ] Implement `lib/ai/index.ts` (`getAiProviderClient`, `AI_TIMEOUT_MS`, `AI_DAILY_SUGGEST_LIMIT`, `ANTHROPIC_MODEL`/`OPENAI_MODEL`) — verify the current cheapest-tier model id against provider docs before pinning
- [ ] Implement `lib/validators/ai-settings.ts` and `lib/validators/categorize-ai.ts` + validator tests
- [ ] Implement `lib/services/aiSettings.ts` (get/save-with-probe/toggles/accept-disclosure/remove, private `loadDecryptedKey`)
- [ ] Unit tests: `tests/unit/services/aiSettings.test.ts` — 401 refuses to persist; 429/5xx persists unverified with warning; masked-only reads; remove is idempotent
- [ ] Implement `lib/services/aiCategorize.ts` (eligibility gate reusing the queue where-clause, `matchCategoryRule` re-check, DB rate limit, provider call, result mapping, disclosure preview)
- [ ] Unit tests: `tests/unit/services/aiCategorize.test.ts` — ineligible row rejected; rule-matched row rejected; toggles honoured; daily cap; each failure mode maps to its typed error; invalid category id resolves to `{ outcome: 'none' }`; zero-category short-circuit makes no network call
- [ ] Add the two `userData` regression tests: export contains no AI fields; restore preserves `UserAiSettings`
- [ ] Implement the four route handlers with `export const runtime = 'nodejs'` and the shared error→status mapping
- [ ] Write `docs/runbooks/rotate-secret-encryption-key.md`
